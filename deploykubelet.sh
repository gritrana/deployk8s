...
# 创建kubelet证书签名请求
echo "=========创建kubelet证书签名请求模板========"
cat > ${KUBELET_PATH}/kubelet-csr.json.template <<EOF
{
...
EOF
ls ${KUBELET_PATH}/kubelet-csr.json.template
...
for ((i=0; i<3; i++))
  do
    ...
    echo "生成kubelet证书签名请求"
    sed -e "s/##NODE_NAME##/${NODE_NAMES[i]}/" \
    ${KUBELET_PATH}/kubelet-csr.json.template > \
    ${KUBELET_PATH}/kubelet-${NODE_IPS[i]}-csr.json
    cat ${KUBELET_PATH}/kubelet-${NODE_IPS[i]}-csr.json
    ...
    -profile=kubernetes \
    ${KUBELET_PATH}/kubelet-${NODE_IPS[i]}-csr.json | \
    cfssljson -bare ${KUBELET_PATH}/kubelet-${NODE_IPS[i]}
    if [ $? -ne 0 ];then echo "生成kubelet证书和私钥失败，退出脚本";exit 1;fi
    ls ${KUBELET_PATH}/kubelet-${NODE_IPS[i]}*.pem
  done
...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    scp ${KUBELET_PATH}/kubelet-${node_ip}.pem \
      root@${node_ip}:/etc/kubernetes/cert/kubelet.pem
    scp ${KUBELET_PATH}/kubelet-${node_ip}-key.pem \
      root@${node_ip}:/etc/kubernetes/cert/kubelet-key.pem
