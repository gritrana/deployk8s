...
cat > kubelet-csr.json.template <<EOF
{
...
for ((i=0; i<3; i++))
  do
    ...
    sed -e "s/##NODE_NAME##/${NODE_NAMES[i]}/" \
      ...
      kubelet-${NODE_IPS[i]}-csr.json
    cat kubelet-${NODE_IPS[i]}-csr.json
    ...
    -profile=kubernetes \
    kubelet-${NODE_IPS[i]}-csr.json | cfssljson -bare kubelet-${NODE_IPS[i]}
    ls kubelet-${NODE_IPS[i]}*.pem
  done
...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    scp kubelet-${node_ip}.pem \
      root@${node_ip}:/etc/kubernetes/cert/kubelet.pem
    scp kubelet-${node_ip}-key.pem \
